...
class CategoryResourceController extends BaseController
{
    ...
    public function create(CategoryRequest $request)
    {
        ...
        $this->response->title(trans('app.new') . ' ' . trans('block::category.name')) 
    ...
    public function store(CategoryRequest $request)
    {
        try {
            ...
            return $this->response->message(trans('messages.success.created', ['Module' => trans('block::category.name')]))
                ->code(204)
                ->status('success')
                ->url(trans_url(guard_url('/block/category/' . $category->getRouteKey())))
                ->redirect();
        } catch (Exception $e) {
            return $this->response->message($e->getMessage())
                ->code(400)
                ->status('error')
                ->url(trans_url(guard_url('/block/category')))
                ->redirect();
        }
